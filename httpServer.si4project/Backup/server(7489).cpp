...
void dealWithGETRequest(int connectFd, const char* fileName)
{
	...
	{
		//回复HTTP响应首部
		//sendRespondHead(connectFd, 200, "OK", "Content-Type: text/plain; charset=iso-8859-1", -1);
		//sendRespondHead(connectFd, 200, "OK", "Content-Type: image/jpeg", stateFile.st_size);
		sendRespondHead(connectFd, 200, "OK", "Content-Type: audio/wav", stateFile.st_size);
